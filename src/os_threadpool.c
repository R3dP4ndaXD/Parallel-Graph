...
void enqueue_task(os_threadpool_t *tp, os_task_t *t)
{
	...
	pthread_mutex_lock(&tp->mutex1);
	list_add_tail(&tp->head, &t->list);
	pthread_mutex_unlock(&tp->mutex1);
...
os_task_t *dequeue_task(os_threadpool_t *tp)
{
	...
	while (queue_is_empty(tp) && !tp->done)
		pthread_cond_wait(&tp->notify, &tp->mutex1);
...
void wait_for_completion(os_threadpool_t *tp)
{
	...
	while (!tp->done)
		pthread_cond_wait(&tp->working, &tp->mutex2);
...
os_threadpool_t *create_threadpool(unsigned int num_threads)
{
	...
	pthread_mutex_init(&tp->mutex1, NULL);
	pthread_mutex_init(&tp->mutex2, NULL);
	pthread_cond_init(&tp->notify, NULL);
	pthread_cond_init(&tp->working, NULL);
	tp->done = 0;
...
void destroy_threadpool(os_threadpool_t *tp)
{
	...
	pthread_mutex_destroy(&tp->mutex1);
